...
class DropDown {
    ...
    constructor(data) {
        this.data = data;
        this.dependentSelects = []; // array of objects, each object representing a dependent select
    }

    getFilteredData(filtersAsArray) {
        return this.data.filter(row => filtersAsArray.every((item, index) => item === row[index]));
    }

    getUniqueValues(dataAsArray, index) {
        const uniqueOptions = new Set();
        dataAsArray.forEach(row => uniqueOptions.add(row[index]));
        return uniqueOptions;
    }

    populateDropDown(el, listAsArray) {  // Helper function used in the createPopulateDropDown method
        el.innerHTML = "";
    ...
    createPopulateDropDownFunction(el, elsDependsOn) {  // closure
        return () => {
            const elsDependsOnValues = elsDependsOn.map(element => element.value);
            const filteredData = elsDependsOn.length === 0 ? this.data : this.getFilteredData(elsDependsOnValues);
            const uniqueOptions = this.getUniqueValues(filteredData, elsDependsOn.length);
            this.populateDropDown(el, uniqueOptions);
        }
    }

    // method that takes in an object representing a select with its associated properties
    addOptions(selectObject) {
        // Store a reference to the element we are targeting
        const el = document.getElementById(selectObject.targetId);

        // Create an array of elements that the dropdown depends on to be filtered
        const elsDependsOn = selectObject.dependsOnArray.map(id => document.getElementById(id));
        
        const populateDropDownEvent = this.createPopulateDropDownFunction(el, elsDependsOn);

        // create an object to be stored in the DropDown object's dependentSelects array
        const dependentSelect = { el: el, elsDependsOn: elsDependsOn, populateDropDownFunction: populateDropDownEvent };

        // Loop through the passed in selectObject's elsDependsOnArray and add an event listener to each item
        elsDependsOn.forEach(item => item.addEventListener('change', populateDropDownEvent));
        this.dependentSelects.push(dependentSelect);
        return this;
    }

    initialize() {
        this.dependentSelects.forEach(object => {
            object.populateDropDownFunction();
        })
        return this;
    }

    build(arrayOfSelectIds) {
        arrayOfSelectIds.forEach( (id, index) => {
            this.addOptions({targetId: id, dependsOnArray: arrayOfSelectIds.slice(0,index)});
        })
        this.initialize();
        return this;
    }
...
let schools = new DropDown(dataSchools).build(['state', 'city', 'school']);
var airports = new DropDown(dataAirports).build(['continent', 'country', 'region', 'municipality', 'airport_type', 'airport']);